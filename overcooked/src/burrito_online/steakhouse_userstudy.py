...
from .encoding.env_enums import (
    counter_mapping,
    ingredient_mapping,
...
from .mdp.burrito_env import BurritoEnv
...
class OvercookedWebApp:
                    ...
                    try:
                        ...
                        while not self.command_queue.empty():
                            ...
                            if command["event"] == "start_game":
                                #    games[game_id]["command_queue"].put({"event": "start_game", "data": data["client_id"]})
                                self.start_now(command["data"])
    ...
    def kickoff_game(self):
        ...
        if not all(
            ...
            return
        player_data = [{"client_id": player["client_id"], "color": player["color"]} for player in self._players]
        self.event_queue.put({"event": "all_joined", "data": player_data, "room": str(self.game_id)})
        return

    def start_now(self, data):
        print("Starting game now")
        print("clicked start game:", data)
        if data:
            for player in self._players:
                if player.get("client_id") == data:
                    player["ready"] = True
                    print("setting ready")
                    print(player)
                    break
        ...
        if all(player.get("ready", False) for player in self._players):
            if not self._main_loop_running:
                for i in range(3, 0, -1):
                    self.event_queue.put(
                        {
                            "event": "countdown",
                            "data": {"message": f"Starting in {i}..."},
                            "room": str(self.game_id),
                        }
                    )
                    socketio.sleep(1)
            ...
            self.event_queue.put(
                        {
                            "event": "ready_count",
                            "data": {"message": f"({sum([player.get('ready', False) for player in self._players])}/{len(self._players)}) Players Ready", 
                                     },
                            "room": str(self.game_id),
                        }
                    )
    ...
    def on_loop(self, fps=10):
        ...
        if time_step > self.prev_timestep:
            ...
            try:  # This is a hacky way to handle the fact that the game might end before the time is up
                ...
                done = self._human_step_env(joint_action)
            except Exception as e:
    ...
    def on_render(self):
        ...
        return result
        # self._hud_data = self.state_visualizer.default_hud_data(
        #     self.env.state,
        #     time_left=self.env.horizon - self.env.state.timestep,
        ...
        #     self.env.state, self.env.mdp.terrain_mtx, hud_data=self._hud_data
        # )
        # # on top of the kitchen, render fog
        # for agent in self.agents:
        #     self.state_visualizer.render_fog(kitchen, self.env, agent)

        # # Create a blank image for the entire grid
        # TILE_SIZE = 64
        # grid_image = Image.new(
        #     "RGB", (self.env.mdp.width * TILE_SIZE, self.env.mdp.height * TILE_SIZE)
        # )
        # for img_id, asset_path in self._last_grid.items():
        #     # Calculate row and column based on image ID

        #     row = img_id // self.env.mdp.width
        #     col = img_id % self.env.mdp.width

        #     asset_load_path = f"./src/html{asset_path}"

        #     # Open the tile image
        #     try:
        #         tile_image = Image.open(asset_load_path).resize((TILE_SIZE, TILE_SIZE))
        #     except FileNotFoundError:
        #         print(
        #             f"Error: File not found for ID {img_id} at path {asset_load_path}"
        #         )
        #         continue
        #     except Exception as e:
        #         print(f"Error loading image {asset_load_path}: {e}")
        #         continue
        ...
        #     # Write text on the tile image
        #     if img_id in self._last_text:
        #         text = self._last_text[img_id]
        #         font = ImageFont.truetype("./src/html/assets/Arial.ttf", 25)
        #         draw = ImageDraw.Draw(tile_image)
        #         draw.text((5, 5), text, (255, 255, 255), font=font)

        #     # Paste the tile image onto the grid
        #     grid_image.paste(tile_image, (x, y))

        # # Finally, write the HUD string on the top of the final image
        # font = ImageFont.truetype("./src/html/assets/Arial.ttf", 15)
        # draw = ImageDraw.Draw(grid_image)
        # hud_string = ("Ordered:\n{}\n" "Score: {}\n" "Time left: {}").format(
        #     ", ".join(
        #         [f'("{order[0]}", "{order[1]}")' for order in self._hud_data["orders"]]
        #     ),
        #     self._hud_data["score"],
        #     self._hud_data["time_left"],
        # )

        # self._env_grids_dic[len(self._env_grids_dic)]["meta"] = np.array(
        #     [self.score, self.env.state.timestep]
        # )

        # draw.text((10, 10), hud_string, (255, 255, 255), font=font)

        # self._recording_images.append(grid_image)

        # Save the final grid image (Debugging only)
        # grid_image.save("image_rendered.png")
    ...
    def _human_step_env(self, joint_action):
        try:
            joint_action_list = []
            for i in joint_action:
                joint_action_list.append(Action.ACTION_TO_INDEX[i])
    ...
    def handle_game_logic(self):
        for i, player in enumerate(self._players):
            if player["type"] != "HUMAN":
                ...
                self.player_actions[i] = action[0]

        online_data = self.on_loop()
    ...
    def handle_key_press(self, cid, key):
        ...
        # Handle logic
        if key == "up":
            self.player_actions[player_index] = Direction.NORTH
        elif key == "right":
            self.player_actions[player_index] = Direction.EAST
        elif key == "down":
            self.player_actions[player_index] = Direction.SOUTH
        elif key == "left":
            self.player_actions[player_index] = Direction.WEST
        elif key == "space":
            self.player_actions[player_index] = Action.INTERACT
    ...
    def convert_action(self, action):
        ...
        return (
            ...
            if action == (0, -1)
                ...
                "DOWN"
                if action == (0, 1)
                else ("LEFT" if action == (-1, 0) else "RIGHT")
            )
        )
    ...
    def _get_current_image(self):
        ...
        for obj in self.env.state.objects.values():
            ...
            if (
                ...
                or (obj.name == "clean_plate" and grid[y_pos][x_pos] == "W")
...
if __name__ == "__main__":
    ...
    # app.debug = True
    socketio.run(app, host="0.0.0.0", port=23000)
