...
class CentralizedActorCritic(TorchModelV2, nn.Module):
    ...
    def value_function(self, input_dict):
        ...
        one_hot_opp_act = torch.nn.functional.one_hot(input_dict['opponent_action'].long(), self.action_space.n).float() #if 'opponent_action' in input_dict else torch.zeros((x.shape[0], self.action_space.n)).float()
        
        if order_embedding.dim() < 2:
            order_embedding = order_embedding.unsqueeze(0)

        team_obs = x # currently environments are fully observable, no need for opponent observations
        x = _compute_layers(team_obs, self.critic_input_layers)
        x = x.reshape(x.shape[0], -1)
        x = torch.concatenate(
            [x, order_embedding, one_hot_opp_act.reshape(one_hot_opp_act.shape[0], -1)], dim=-1
        )
